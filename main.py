...
model_used = 'HM_model_over_night.pth'
...
def process_images(image_directory, proc_settings):
    ...
    for image_path in image_list:
        try:
            ...
            logger.error('Failed opening file: ' + image_path)
            img = None
...
@logger.catch
def publish_to_slate(expected_concentration, pub, timestamp):
    ...
    pub.msg.epochMillisec = int((timestamp - processing_interval/2) * 1000)
...
if __name__=="__main__":
    ...
    images_to_delete = glob.glob(os.path.join(image_directory,'*.tif'))
    ...
    while True:
        ...
        quants = quantify_images(labels, processing_interval)
        publish_to_slate(quants, pub, current_time)
